...
class Outs_Calculator:
    ...
    def evalue_hand(self, pocket):
        card_ranks_original = "23456789TJQKA"
        rcounts = {card_ranks_original.find(r): ''.join(pocket).count(r) for r, _ in pocket}.keys()
        return rcounts
    ...
    def calc_score(self, hand):  # assign a calc_score to the hand so it can be compared with other hands
        ...
        if score[0] == 5:
            ...
            hand_type,kicker = self.getKicker(card_ranks,rcounts)
    ...
    def getKicker(self,card_ranks,rcounts):

        kicker =  0
        maxCard=0
        board=self.evalue_hand(self.board);
        ...
        for cardCount in rcounts:
            if cardCount[1]==2:
                pairedCard=cardCount[0]
        ...
        if pairedCard >= maxCard:
            type="TP"

        if kicker == 12:
            kicker = "TK"
        elif kicker == 11 or kicker == 10:
            kicker = "GK"
        else:
            kicker = "LK"
    ...
    def anaylyse_flop(self, pocket, board,hero_position,opponent_position, oc):
        oc.hand = pocket + board
        oc.board = board
        oc.pocket = pocket
    ...
    def hand_type(self, pocket, board, oc):
        oc.hand = pocket + board
        oc.board = board
        oc.pocket = pocket
        ...
        if oc.hand_result =='HighCard' or 'TP' in oc.hand_result :

            outs = self.calculate_outsWithoutdeck(oc)

            if outs == 8:
                if 'TP' in oc.hand_result:
                    oc.hand_result = "TP_open_ended_straight_draw";
                else:
                    oc.hand_result = "open_ended_straight_draw";


            if outs == 9:
                if 'TP' in oc.hand_result:
                    oc.hand_result = "TP_flush_draw";
                else:
                    oc.hand_result = "flush_draw";
            ...
            if outs == 4:
                if 'TP' in oc.hand_result:
                    oc.hand_result = "TP_gut_shot_straight_draw";
                else:
                  oc.hand_result = "gut_shot_straight_draw";

            if outs == 12:
                if 'TP' in oc.hand_result:
                    oc.hand_result = "TP_gut_shot_straight_flush_draw";
                else:
                    oc.hand_result = "gut_shot_straight_flush_draw";


            if outs == 15:
                if 'TP' in oc.hand_result:
                    oc.hand_result = "TP_open_straight_and_flush_draw";
                else:
                    oc.hand_result = "open_straight_and_flush_draw";


        return oc.hand_result
    ...
    def calculate_outsWithoutdeck(self, oc):
        outs = 0
        oc.deck = self.create_card_deck(oc)
        # Use functions to calculate outs
        outs_list = [0] * 4
